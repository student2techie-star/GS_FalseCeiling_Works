    ...
    <div className="h-[calc(100vh-6rem)] flex flex-col md:flex-row gap-6 pb-6">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <div className="glass rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-white/60 p-4 mb-6 flex justify-between items-center shrink-0">
          ...
          <div className="flex items-center gap-3">
            <button onClick={saveInvoice} disabled={saving} className="bg-gradient-to-r from-primary to-accent text-white px-5 py-2.5 rounded-xl font-bold flex items-center gap-2 hover:shadow-lg hover:shadow-accent/20 hover:-translate-y-0.5 transition-all disabled:opacity-50 disabled:hover:translate-y-0">
            ...
            <div className="mt-8 border-t border-slate-200/50 pt-6">
              <div className="flex justify-between items-center mb-6">
                <h3 className="font-heading font-extrabold text-xl text-primary flex items-center gap-2">
                  <CreditCard className="w-6 h-6 text-accent" /> Payments Received
                </h3>
                {balanceDue > 0 ? (
                   <button onClick={addPayment} className="text-white bg-emerald-500 hover:bg-emerald-600 px-4 py-2 rounded-lg text-sm font-bold shadow-sm flex items-center gap-2 transition-colors">
                     <Plus className="w-4 h-4" /> Record Payment
                   </button>
                ) : (
                   <span className="text-emerald-700 font-extrabold text-sm bg-emerald-100/80 px-4 py-1.5 rounded-full border border-emerald-200 shadow-sm">PAID IN FULL</span>
                )}
              </div>
